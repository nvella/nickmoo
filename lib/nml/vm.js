...
VM.prototype.resolveValue = function(value, callback) {
  if(typeof(value) !== 'object') return callback(value); // This is a direct value, just return it.
  switch(value.type) {
    case 'var': // Local variable, return it from our state.
      return callback(this.state.localVars[value.name]);
    ...
    case 'objectIdAlias': // This is an object id alias, ask the app for what it means
      return callback(this._app.resolveObjIdAlias(value.value));
    default:
...
VM.prototype.evalExpr = function(expr, callback) {
  ...
  if(OPERATORS.indexOf(expr[1]) < 0) {
    ...
    if(noObjects) return callback(expr.join(' '));
  }
  ...
  var parseNextComponent = function(i) {
    if(i >= expr.length) {
      ...
      switch(op) {
        case '+':
          this.resolveValue(expr[i], function(value) { output += value; parseNextComponent(i + 2); });
          break;
        case '-':
          this.resolveValue(expr[i], function(value) { output -= value; parseNextComponent(i + 2); });
          break;
      ...
      this.resolveValue(expr[i], function(value) {
        output = value;
...
VM.prototype.stepOnce = function(callback) {
  ...
  switch(ins.type) {
    ...
    case 'if':
      break;
    case 'assign':
      var exprVal = this.evalExpr(ins.src);
      if(ins.op === '=') {
        // Simple assignment
        if(ins.dst.type === 'var') {
