...
var OPERATORS = {'+':   function(left, right) { return left + right; },
  ...
  var parseNextComponent = function(i) {
    if(i >= expr.length) {
      ...
      this.resolveValue(expr[i], function(err, value) {
        ...
        if(TYPE_COMPATIBILITY[op] !== undefined && [op][typeof(value)].indexOf(typeof(output)) < 0) return callback('type ' + typeof(value) + ' cannot be opped ' + op + ' with ' + typeof(output));
