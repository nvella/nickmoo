...
VM.prototype.evalExpr = function(expr, callback) {
  ...
  var parseNextComponent = function(i) {
    if(i >= expr.length) {
      ...
      var op = expr[i - 1];
      if(OPERATORS.indexOf(op) < 0) return callback(op + ' is not a valid operator (+ - / *)', null);
      if(TYPE_COMPATIBILITY[op][typeof(expr[i])].indexOf(typeof(output)) < 0) return callback('type ' + typeof(expr[i]) + ' cannot be opped ' + op + ' with ' + typeof(output), null);
      switch(op) {
        case '+':
          this.resolveValue(expr[i], function(err, value) { output += value; parseNextComponent(i + 2); });
          break;
        case '-':
          this.resolveValue(expr[i], function(err, value) { output -= value; parseNextComponent(i + 2); });
          break;
      ...
      function(value, c) {
        if(ins.op === '=') {
          ...
          if(ins.dst.type === 'var') {
            ...
            this.state.localVar[ins.dst.name] = value;
            c();
