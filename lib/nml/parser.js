...
var NMLSyntaxError       = require('./errors').NMLSyntaxError;
      ...
      default:
        ...
        switch(line[i]) {
          ...
          case ']': // Close array/list
            if(INDEX_SPECIFIABLE.indexOf(ctxStack.slice(-1)[0].type) < 0) throw new NMLSyntaxError('line ' + lineNo + ': last opened context in line was not an array or array index specifier');
            // Merge the top context down into the context below it
            ctxStack.slice(-2)[0].ctx.push(ctxStack.slice(-1)[0]);
            // Pop off the array from the context stack
            ctxStack.pop();
...
Parser.parseGroup = function(groupConcat, level, lineNo) {
  ...
  var err = function(str) {
    throw new NMLSyntaxError('line ' + lineNo + ': ' + str);
  };
...
Parser.codeToAst = function(code) {
  ...
  for(var lineNo in lines) {
    ...
    var err = function(str) {
      throw new NMLSyntaxError('line ' + (parseInt(lineNo) + 1) + ': ' + str);
    };
  ...
  if(blockStack.length > 1) throw new NMLSyntaxError((blockStack.length - 1) + ' block(s) are still open.');
