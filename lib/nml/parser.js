...
Parser.parseGroup = function(groupConcat, level, lineNo) {
  ...
  // Split the group on operators, and run the below for each
  var buffer = [];
  var subGroups = [];
  for(var com of groupConcat) {
    if(typeof(com) === 'string' && OPERATORS[com] !== undefined) {
      subGroups.push({tailop: com, group: buffer, result: []});
      buffer = [];
    } else {
      buffer = buffer.concat(com);
    }
  }
  if(buffer.length > 0) subGroups.push({tailop: null, group: buffer, result: []});

  function processSubgroup(obj) {
    var parsed = obj.group;
    ...
    for(var com of parsed) {
      if(com.type === 'group' || com.type === 'array') {
        ...
        if(com.type === 'group' && typeof(parsed[i - 1]) === 'string') {
    ...
    if(typeof(parsed[0]) === 'string' && level === 0) {
      var verbcall;
      return Parser.verbcallFromParsedLine(parsed, level + 1, lineNo);
    ...
    if(ASSIGNMENT_OPERATORS.indexOf(parsed[1]) >= 0) {
      ...
      var stmt = {
        type: 'assign',
        op: parsed[1][0],
        src: Parser.parseGroup(parsed.slice(2, group.length - (commentExists ? 1 : 0)), level + 1, lineNo),
        dst: parsed[0]
      };
  ...
  // Piece back together the subgroups
  var out = []
  for(var group of subGroups) {
    out = out.concat(processSubgroup(group));
    if(group.tailop !== null) out.push(group.tailop);
  }
  return out;
}
  ...
  for(var lineNo in lines) {
    ...
    switch(initialKeyword) {
      ...
      case 'if':
        ...
        var statement = {
          type: initialKeyword,
          expr: Parser.parseGroup(line.slice(1, line.length - (commentExists ? 1 : 0)), 0, lineNo),
          block: []
        };
    ...
    blockStack.slice(-1)[0].push(Parser.parseGroup(line, 0, lineNo)[0]);
  }
