  ...
  for(var lineNo in lines) {
    ...
    switch(initialKeyword) {
      ...
      case 'if':
        ...
        var statement = {
          type: initialKeyword,
          expr: line.slice(1, line.length - (commentExists ? 1 : 0)),
          block: []
        };
    ...
    var verbcall;
    if(line[1] !== undefined && line[1].type === 'group') {
      verbcall = Parser.verbcallFromParsedLine([initialKeyword].concat(line[1].ctx));
    } else {
      verbcall = Parser.verbcallFromParsedLine(line);
    }

    // Push the statement onto the block stack
    blockStack.slice(-1)[0].push(verbcall);
  }
