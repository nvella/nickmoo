  ...
  async.series([
    ...
    function(callback) {
      ...
      this.getParent(function(err, parent) {
        async.each(['directObj', 'indirectObj'], function(objRef, callback) {
          if(verbcallObj[objRef] instanceof Array) verbcallObj[objRef] = verbcallObj[objRef].join(' ');
    ...
    function(callback) {
      ...
      this.getParent(function(err, parent) {
        if(err) return callback(err);
        if(parent === null) return callback();
        parent.vmFromVerb(verbcallObj.verb, function(err, vm) {
          if(err) {
            if(err.message === 'verb does not exist') return callback();
            return callback(err);
          }
    ...
    function(callback) {
      ...
      this.getParent(function(err, parent) {
        if(err) return callback(err);
        if(parent === null) return callback();
        parent.getChildren(function(err, children) {
          if(err) return callback(err);
          async.each(children, function(child, callback) {
            // TODO only accept children whose name is referenced as either the direct or indirect obj
            child.vmFromVerb(verbcallObj.verb, function(err, vm) {
              if(err) {
                if(err.message === 'verb does not exist') return callback();
                return callback(err);
              }
