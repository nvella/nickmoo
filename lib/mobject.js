...
MObject.prototype.load = function(callback) {
  this._app.collections.objects.findOne({_id: this.id}, function(err, result) {
    ...
    this.created = doc._created;

    // Load properties
    for(var prop in doc) {
      if(prop[0] === '_') continue; // Skip document properties that start with _ (private)
      this.props[prop] = doc[prop];
    }

    // Load verbs
    for(var verb in doc._verbs) {
      this.verbs[verb] = {
        src: doc._verbs[verb],
